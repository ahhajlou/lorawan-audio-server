import sys

from loguru import logger

import exceptions
from config import load_env_variables
from dispatch import DispatcherManager
from handlers import JoinHandler, TxAckHandler, UplinkHandler
from registry import DeviceRegistry
from routing import Forwarder, GatewayDispatchQueue
from transport.mqtt_client import MqttTransport

logger.remove()


@logger.catch
def main():
    settings = load_env_variables()

    logger.add(sys.stderr, level=settings.log_level)
    logger.info("Hello from lorawan-audio-server!")

    mqtt_transport = MqttTransport(settings=settings)

    registry = DeviceRegistry()
    dispatch_queue = GatewayDispatchQueue()
    forwarder = Forwarder(registry)
    dispatcher_manager = DispatcherManager(
        dispatch_queue=dispatch_queue,
        publish_downlink=mqtt_transport.publish_downlink,
        txack_timeout=settings.txack_timeout_s,
    )

    uplink_handler = UplinkHandler(forwarder, dispatch_queue, dispatcher_manager)
    join_handler = JoinHandler(forwarder)
    txack_handler = TxAckHandler(dispatcher_manager)

    mqtt_transport.set_uplink_handler(uplink_handler.handle)
    mqtt_transport.set_join_handler(join_handler.handle)
    mqtt_transport.set_txack_handler(txack_handler.handle)

    try:
        mqtt_transport.start()
    except exceptions.MQTTConnectionError:
        logger.error("Exiting — MQTT connection error")
        sys.exit(1)
    except KeyboardInterrupt:
        logger.warning("Keyboard interrupt, exiting")
        sys.exit(2)

    try:
        mqtt_transport.loop_forever()
    except exceptions.MQTTConnectionError:
        logger.error("Exiting — MQTT connection error")
        sys.exit(1)
    except KeyboardInterrupt:
        logger.warning("Keyboard interrupt, exiting")
        dispatcher_manager.stop_all()
        try:
            mqtt_transport.stop()
        except exceptions.MQTTConnectionError:
            logger.error("Exiting — MQTT connection error")
        sys.exit(2)


if __name__ == "__main__":
    main()
